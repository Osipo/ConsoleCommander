...
namespace ConsoleCommander.Structures {
    class LinkedDictionary<K,V> : IDictionary<K,V>, ICollection<KeyValuePair<K,V>>, IEnumerable<KeyValuePair<K,V>>, IEnumerable {
        ...
        public LinkedDictionary(){
            this._count = 0;
            this._entries = new ConsoleCommander.Structures.LinkedList<KeyValuePair<K,V>>();
        ...
        public System.Collections.Generic.ICollection<K> Keys{
            get{
                ConsoleCommander.Structures.LinkedList<K> lks = new ConsoleCommander.Structures.LinkedList<K>();
                foreach(KeyValuePair<K,V> entry in _entries){
                    lks.Add(entry.Key);
                }
                return lks;
        ...
        public System.Collections.Generic.ICollection<V> Values {
            get{
                ConsoleCommander.Structures.LinkedList<V> lks = new ConsoleCommander.Structures.LinkedList<V>();
                foreach(KeyValuePair<K,V> entry in _entries){
                    lks.Add(entry.Value);
                }
                return lks;
